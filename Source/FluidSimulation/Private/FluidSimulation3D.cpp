...
void FluidSimulation3D::UpdateDiffusion() const
{
    ...
    {
        auto scaledDiffusion = mp_velocity->Properties()->diffusion / static_cast<float>(m_diffusionIter);
...
void FluidSimulation3D::ReverseAdvection(TSharedPtr<Fluid3D, ESPMode::ThreadSafe> p_in, TSharedPtr<Fluid3D, ESPMode::ThreadSafe> p_out, float scale) const
{
    auto force = m_dt * scale; // distance to advect
    float vx, vy, vz; // velocity values of the current x,y,z locations
    float x1, y1, z1; // x, y, z locations after advection
    ...
    *p_out = *p_in;
    ...
    TArray3D<int32> FromSource_xA(m_size_x, m_size_y, m_size_z, -1); // The new X coordinate after advection stored in x,y,z where x,y,z,z is the original source point
    TArray3D<int32> FromSource_yA(m_size_x, m_size_y, m_size_z, -1); // The new Y coordinate after advection stored in x,y,z where x,y,z is the original source point
    TArray3D<int32> FromSource_zA(m_size_x, m_size_y, m_size_z, -1); // The new Z coordinate after advection stored in x,y,z where x,y,z is the original source point
    TArray3D<float> FromSource_A(m_size_x, m_size_y, m_size_z); // The value of A after advection stored in x,y,z where x,y,z is the original source point
    TArray3D<float> FromSource_B(m_size_x, m_size_y, m_size_z); // The value of B after advection stored in x,y,z where x,y,z is the original source point
    TArray3D<float> FromSource_C(m_size_x, m_size_y, m_size_z); // The value of C after advection stored in x,y,z where x,y,z is the original source point
    TArray3D<float> FromSource_D(m_size_x, m_size_y, m_size_z); // The value of D after advection stored in x,y,z where x,y,z is the original source point
    TArray3D<float> FromSource_E(m_size_x, m_size_y, m_size_z); // The value of E after advection stored in x,y,z where x,y,z is the original source point
    TArray3D<float> FromSource_F(m_size_x, m_size_y, m_size_z); // The value of F after advection stored in x,y,z where x,y,z is the original source point
    TArray3D<float> FromSource_G(m_size_x, m_size_y, m_size_z); // The value of G after advection stored in x,y,z where x,y,z is the original source point
    TArray3D<float> FromSource_H(m_size_x, m_size_y, m_size_z); // The value of H after advection stored in x,y,z where x,y,z is the original source point
    TArray3D<float> TotalDestValue(m_size_x, m_size_y, m_size_z); // The total accumulated value after advection stored in x,y,z where x,y,z is the destination point
...
void FluidSimulation3D::Diffusion(TSharedPtr<Fluid3D, ESPMode::ThreadSafe> p_in, TSharedPtr<Fluid3D, ESPMode::ThreadSafe> p_out, const float scale) const
{
        ...
        for (int32 z = 1; z < m_size_z - 1; z++)
        {
            ...
            p_out->element(x, m_size_y - 1, z) = p_in->element(x, m_size_y - 1, z) + force *
                (p_in->element(x - 1, m_size_y - 1, z) +
                    ...
                    p_in->element(x, m_size_y - 1, z + 1) +
                    p_in->element(x, m_size_y - 2, z) - 5.0f * p_in->element(x, m_size_y - 1, z));
        }
    }
        ...
        for (int32 z = 1; z < m_size_z - 1; z++)
        {
            ...
            p_out->element(m_size_x - 1, y, z) = p_in->element(m_size_x - 1, y, z) + force *
                (p_in->element(m_size_x - 1, y - 1, z) +
                    ...
                    p_in->element(m_size_x - 1, y, z + 1) +
                    p_in->element(m_size_x - 2, y, z) - 5.0f * p_in->element(m_size_x - 1, y, z));
        }
    }
        ...
        for (int32 y = 1; y < m_size_y - 1; y++)
        {
            ...
            p_out->element(x, y, m_size_z - 1) = p_in->element(x, y, m_size_z - 1) + force *
                (p_in->element(x, y - 1, m_size_z - 1) +
                    ...
                    p_in->element(x + 1, y, m_size_z - 1) +
                    p_in->element(x, y, m_size_z - 2) - 5.0f * p_in->element(x, y, m_size_z - 1));
        }
    }
    ...
    for (int32 x = 1; x < m_size_x - 1; x++)
    {
        ...
        p_out->element(x, m_size_y - 1, 0) = p_in->element(x, m_size_y - 1, 0) + force *
            (p_in->element(x - 1, m_size_y - 1, 0) +
                ...
                p_in->element(x, m_size_y - 1, 1) - 4.0f * p_in->element(x, m_size_y - 1, 0));
        p_out->element(x, 0, m_size_z - 1) = p_in->element(x, 0, m_size_z - 1) + force *
            (p_in->element(x - 1, 0, m_size_z - 1) +
                p_in->element(x + 1, 0, m_size_z - 1) +
                p_in->element(x, 0, m_size_z - 2) +
                p_in->element(x, 1, m_size_z - 1) - 4.0f * p_in->element(x, 0, m_size_z - 1));
        p_out->element(x, m_size_y - 1, m_size_z - 1) = p_in->element(x, m_size_y - 1, m_size_z - 1) + force *
            (p_in->element(x - 1, m_size_y - 1, m_size_z - 1) +
                ...
                p_in->element(x, m_size_y - 1, m_size_z - 2) - 4.0f * p_in->element(x, m_size_y - 1, m_size_z - 1));
    }
    ...
    for (int32 y = 1; y < m_size_y - 1; y++)
    {
        ...
        p_out->element(m_size_x - 1, y, 0) = p_in->element(m_size_x - 1, y, 0) + force *
            (p_in->element(m_size_x - 1, y - 1, 0) +
                ...
                p_in->element(m_size_x - 2, y, 0) - 4.0f * p_in->element(m_size_x - 1, y, 0));
        p_out->element(0, y, m_size_z - 1) = p_in->element(0, y, m_size_z - 1) + force *
            (p_in->element(0, y - 1, m_size_z - 1) +
                p_in->element(0, y + 1, m_size_z - 1) +
                p_in->element(0, y, m_size_z - 2) +
                p_in->element(1, y, m_size_z - 1) - 4.0f * p_in->element(0, y, m_size_z - 1));
        p_out->element(m_size_x - 1, y, m_size_z - 1) = p_in->element(m_size_x - 1, y, m_size_z - 1) + force *
            (p_in->element(m_size_x - 1, y - 1, m_size_z - 1) +
                ...
                p_in->element(m_size_x - 2, y, m_size_z - 1) - 4.0f * p_in->element(m_size_x - 1, y, m_size_z - 1));
    }
    ...
    for (int32 z = 1; z < m_size_z - 1; z++)
    {
        ...
        p_out->element(m_size_x - 1, 0, z) = p_in->element(m_size_x - 1, 0, z) + force *
            (p_in->element(m_size_x - 1, 0, z - 1) +
                ...
                p_in->element(m_size_x - 1, 1, z) - 4.0f * p_in->element(m_size_x - 1, 0, z));
        p_out->element(0, m_size_y - 1, z) = p_in->element(0, m_size_y - 1, z) + force *
            (p_in->element(0, m_size_y - 1, z - 1) +
                p_in->element(0, m_size_y - 1, z + 1) +
                p_in->element(1, m_size_y - 1, z) +
                p_in->element(0, m_size_y - 2, z) - 4.0f * p_in->element(0, m_size_y - 1, z));
        p_out->element(m_size_x - 1, m_size_y - 1, z) = p_in->element(m_size_x - 1, m_size_y - 1, z) + force *
            (p_in->element(m_size_x - 1, m_size_y - 1, z - 1) +
                ...
                p_in->element(m_size_x - 1, m_size_y - 2, z) - 4.0f * p_in->element(m_size_x - 1, m_size_y - 1, z));
    }
    ...
    p_out->element(m_size_x - 1, 0, 0) = p_in->element(m_size_x - 1, 0, 0) + force *
        (p_in->element(m_size_x - 2, 0, 0) +
            ...
            p_in->element(m_size_x - 1, 0, 1) - 3.0f * p_in->element(m_size_x - 1, 0, 0));
    p_out->element(0, m_size_y - 1, 0) = p_in->element(0, m_size_y - 1, 0) + force *
        (p_in->element(1, m_size_y - 1, 0) +
            p_in->element(0, m_size_y - 2, 0) +
            p_in->element(0, m_size_y - 1, 1) - 3.0f * p_in->element(0, m_size_y - 1, 0));
    p_out->element(m_size_x - 1, m_size_y - 1, 0) = p_in->element(m_size_x - 1, m_size_y - 1, 0) + force *
        (p_in->element(m_size_x - 2, m_size_y - 1, 0) +
            ...
            p_in->element(m_size_x - 1, m_size_y - 1, 1) - 3.0f * p_in->element(m_size_x - 1, m_size_y - 1, 0));
    p_out->element(0, 0, m_size_z - 1) = p_in->element(0, 0, m_size_z - 1) + force *
        (p_in->element(1, 0, m_size_z - 1) +
            p_in->element(0, 1, m_size_z - 1) +
            p_in->element(0, 0, m_size_z - 2) - 3.0f * p_in->element(0, 0, m_size_z - 1));
    p_out->element(m_size_x - 1, 0, m_size_z - 1) = p_in->element(m_size_x - 1, 0, m_size_z - 1) + force *
        (p_in->element(m_size_x - 2, 0, m_size_z - 1) +
            ...
            p_in->element(m_size_x - 1, 0, m_size_z - 2) - 3.0f * p_in->element(m_size_x - 1, 0, m_size_z - 1));
    p_out->element(0, m_size_y - 1, m_size_z - 1) = p_in->element(0, m_size_y - 1, m_size_z - 1) + force *
        (p_in->element(1, m_size_y - 1, m_size_z - 1) +
            ...
            p_in->element(0, m_size_y - 1, m_size_z - 2) - 3.0f * p_in->element(0, m_size_y - 1, m_size_z - 1));
    p_out->element(m_size_x - 1, m_size_y - 1, m_size_z - 1) = p_in->element(m_size_x - 1, m_size_y - 1, m_size_z - 1) + force *
        (p_in->element(m_size_x - 2, m_size_y - 1, m_size_z - 1) +
            p_in->element(m_size_x - 1, m_size_y - 2, m_size_z - 1) +
            p_in->element(m_size_x - 1, m_size_y - 1, m_size_z - 2) - 3.0f * p_in->element(m_size_x - 1, m_size_y - 1, m_size_z - 1));
...
void FluidSimulation3D::DiffusionStable(TSharedPtr<Fluid3D, ESPMode::ThreadSafe> p_in, TSharedPtr<Fluid3D, ESPMode::ThreadSafe> p_out, const float scale) const
{
    float force = m_dt * scale;
...
void FluidSimulation3D::ReverseSignedAdvection(TSharedPtr<VelPkg3D, ESPMode::ThreadSafe> v, const float advectScale) const
{
...
void FluidSimulation3D::ExponentialDecay(TSharedPtr<Fluid3D, ESPMode::ThreadSafe> p_in_and_out, const float decay) const
{
    for (int32 x = 0; x < m_size_x; x++)
    {
        for (int32 y = 0; y < m_size_y; y++)
...
void FluidSimulation3D::InvertVelocityEdges() const
{
    for (int32 y = 0; y < m_size_y; y++)
    {
        for (int32 z = 0; z < m_size_z; z++)
        {
            ...
            {
                mp_velocity->SourceX()->element(m_size_x - 1, y, z) = -mp_velocity->SourceX()->element(m_size_x - 1, y, z);
            }
        }
    }
    ...
    for (int32 x = 0; x < m_size_x; x++)
    {
        for (int32 z = 0; z < m_size_z; z++)
        {
            ...
            {
                mp_velocity->SourceY()->element(x, m_size_y - 1, z) = -mp_velocity->SourceY()->element(x, m_size_y - 1, z);
            }
        }
    }
    ...
    for (int32 x = 0; x < m_size_x; x++)
    {
        for (int32 y = 0; y < m_size_y; y++)
        {
            ...
            {
                mp_velocity->SourceZ()->element(x, y, m_size_z - 1) = -mp_velocity->SourceZ()->element(x, y, m_size_z - 1);
            }
        }
    }
}
